...
export async function runClientCommand(sessionId: string, profile?: string, wait?: boolean): Promise<void> {
  const gatewayUrl = process.env.MCP_MUX_GATEWAY_URL ?? `http://${process.env.MCP_MUX_HOST ?? '127.0.0.1'}:${process.env.MCP_MUX_PORT ?? '8787'}`;

  try {
    await runStdioClientAdapter({ sessionId, gatewayUrl, profile, wait });
  } catch (error) {
